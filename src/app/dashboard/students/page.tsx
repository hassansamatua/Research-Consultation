...
import { 
  MinimalHeader
} from '@/components/ui/MinimalHeader';
import { 
...
export default function CleanStudentsPage() {
  const router = useRouter();
  const [user, setUser] = useState<any>(null);
  const [students, setStudents] = useState<Student[]>([]);
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState<'ultra-clean' | 'image-style' | 'table-style'>('ultra-clean');
          ...
          <ResponsiveCard>
            <div className="p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <ResponsiveButton
                  variant="outline"
                  onClick={() => router.push('/dashboard/students/navigation')}
                >
